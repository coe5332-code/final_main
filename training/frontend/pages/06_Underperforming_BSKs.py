...
if st.session_state.filters:
    with st.spinner("Fetching BSK performance data..."):
        ...
        sort_option = st.selectbox(
            "Sort BSKs by Score", ["Lowest Score", "Highest Score"], index=0
        )
    ...
    if not df.empty:
        ...
        with col2:
            # Block distribution
            block_counts = (
                df["block_municipalty_name"].value_counts().head(10).reset_index()
            )
        ...
        lat_cols = [
            col
            for col in df.columns
            if any(keyword in col.lower() for keyword in ["lat", "latitude"])
        ]
        lng_cols = [
            col
            for col in df.columns
            if any(keyword in col.lower() for keyword in ["long", "lng", "longitude"])
        ]
        ...
        if lat_cols and lng_cols:
            ...
            if not map_df.empty:
                ...
                fig_map = px.scatter_mapbox(
                    ...
                    lon=lng_col,
                    color=(
                        "district_name" if "district_name" in available_cols else None
                    ),
                    hover_name="bsk_name" if "bsk_name" in available_cols else None,
                    hover_data={
                        "district_name": (
                            True if "district_name" in available_cols else False
                        ),
                        "block_municipalty_name": (
                            True
                            if "block_municipalty_name" in available_cols
                            else False
                        ),
                    },
                    zoom=6,
                ...
                with col1:
                    ...
                    st.metric(
                        "Latitude Range",
                        f"{map_df[lat_col].min():.4f} to {map_df[lat_col].max():.4f}",
                    )
                with col2:
                    st.metric(
                        "Unique Districts",
                        (
                            map_df["district_name"].nunique()
                    ...
                    st.metric(
                        "Longitude Range",
                        f"{map_df[lng_col].min():.4f} to {map_df[lng_col].max():.4f}",
                    )
            else:
            ...
            for i, col in enumerate(df.columns, 1):
                st.write(f"{i}. {col}")
            st.info(
                "💡 **Tip:** Make sure your dataset includes latitude and longitude columns. Common names include: lat, latitude, long, lng, longitude, bsk_lat, bsk_long, etc."
            )
            ...
            with st.expander(f"{bsk_info['bsk_name']}", expanded=False):
                ...
                with col2:
                    ...
                    if "bsk_lat" in bsk_info and "bsk_long" in bsk_info:
                        st.write(
                            f"Coordinates: {bsk_info['bsk_lat']}, {bsk_info['bsk_long']}"
                        )
