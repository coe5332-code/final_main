...
with tab1:
    ...
    bsk_options = (
        ...
        .apply(
            lambda x: f"{x.bsk_name} (ID: {x.bsk_id}, Priority: {x.priority_score:.1f})",
            axis=1,
        )
        .tolist()
    )
    ...
    if selected_bsk:
        ...
        with col3:
            ...
            priority_class = (
                "priority-high"
                if priority > 70
                else "priority-medium" if priority > 40 else "priority-low"
            )
            st.markdown(
                f'<div class="{priority_class}">Priority: {priority:.1f}</div>',
                unsafe_allow_html=True,
            )
        with col4:
            st.metric(
                "Training Services Needed", int(bsk_data["total_training_services"])
            )
        ...
        with col2:
            ...
            st.write(
                f"**Services Needing Training:** {int(bsk_data['total_training_services'])}"
            )
...
with tab3:
    ...
    if service_rows:
        ...
        with col2:
            ...
            type_summary = (
                service_summary.groupby("service_type")["bsk_count"].sum().reset_index()
            )
            fig2 = px.pie(
                type_summary,
                values="bsk_count",
...
with tab4:
    ...
    if (
        show_map
        and "bsk_lat" in filtered_df.columns
        and "bsk_long" in filtered_df.columns
    ):
        map_df = filtered_df[
            ["bsk_name", "district_name", "bsk_lat", "bsk_long", "priority_score"]
        ].copy()
...
with tab5:
    ...
    with col3:
        # District summary
        csv_district = (
            district_summary.to_csv(index=False).encode("utf-8")
