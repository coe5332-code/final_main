...
def fetch_training_recommendations():
    ...
    try:
        response = requests.get(
            ...
            params={'limit': 1000, 'summary_only': False},
        ...
        if e.response.status_code == 422:
            st.warning(f"⚠️ API parameter validation error. Using smaller limit.")
            # Try with smaller limit
            try:
                response = requests.get(
                    f"{API_BASE_URL}/service_training_recomendation/",
                    params={'limit': 100, 'summary_only': False},
                    timeout=30
                )
                response.raise_for_status()
                return response.json()
            except Exception as e2:
                st.error(f"Error with reduced limit: {e2}")
                return []
        else:
...
def calculate_coverage_metrics(services, videos_df):
    ...
    services_with_videos = videos_df['service_id'].nunique() if not videos_df.empty else 0
    coverage_pct = (services_with_videos / total_services * 100) if total_services > 0 else 0
...
with st.sidebar:
    ...
    view_mode = st.radio(
        "View Mode",
        ["📈 Executive Summary", "🎬 Video Lifecycle", "📊 Analytics", "⚠️ Alerts & Actions"],
...
if view_mode == "📈 Executive Summary":
    ...
    with col1:
        ...
        coverage_data = pd.DataFrame({
            ...
            'Count': [coverage_metrics['services_with_videos'], 
    ...
    if not videos_df.empty:
        ...
        for _, video in recent_videos.iterrows():
            service_info = services_df[services_df['service_id'] == video['service_id']].iloc[0] if not services_df.empty else {}
            service_name = service_info.get('service_name', f"Service {video['service_id']}")
            
            status_badge = f'<span class="status-new">NEW</span>' if video['is_new'] else f'<span class="status-old">OLD</span>'
...
# ============================================================
    ...
    with col2:
        new_count = len(videos_df[videos_df['is_new'] == True]) if not videos_df.empty else 0
    ...
    if not videos_df.empty:
        ...
        timeline_data = videos_df.groupby('created_date').size().reset_index(name='count')
    ...
    if not services_df.empty and not videos_df.empty:
        ...
        services_without_videos = services_df[~services_df['service_id'].isin(services_with_videos)]
        
        if not services_without_videos.empty:
            st.dataframe(
                services_without_videos[['service_id', 'service_name', 'service_type', 'department_name']],
    ...
    if not videos_df.empty:
        ...
        version_counts = videos_df.groupby('service_id').size().reset_index(name='version_count')
        ...
        if not multi_version.empty:
            multi_version_merged = multi_version.merge(
                services_df[['service_id', 'service_name']], 
                on='service_id', 
                how='left'
            )
            multi_version_merged = multi_version_merged.sort_values('version_count', ascending=False)
...
# ============================================================
        ...
        if not videos_df.empty:
            ...
            with col2:
                ...
                fig = px.histogram(
                    videos_df,
                    x='size_mb',
                    title='Video Size Distribution',
                    nbins=20
                )
                st.plotly_chart(fig, use_container_width=True)
            
            # Monthly production trend
            videos_df['month'] = pd.to_datetime(videos_df['created_at']).dt.to_period('M')
    ...
    with tab2:
        ...
        if not services_df.empty:
            ...
            if 'department_name' in services_df.columns:
                dept_coverage = services_df.groupby('department_name').agg({
                    'service_id': 'count'
                }).reset_index()
                ...
                if not videos_df.empty:
                    ...
                    dept_videos = videos_by_dept.groupby('department_name').size().reset_index(name='Videos')
                    dept_coverage = dept_coverage.merge(dept_videos, left_on='Department', right_on='department_name', how='left')
                    dept_coverage['Videos'] = dept_coverage['Videos'].fillna(0).astype(int)
                    dept_coverage['Coverage %'] = (dept_coverage['Videos'] / dept_coverage['Services'] * 100).round(1)
                
                st.dataframe(dept_coverage, use_container_width=True, hide_index=True)
    ...
    with tab3:
        ...
        if not recommendations_df.empty:
            ...
            with col3:
                total_training_needed = recommendations_df['total_training_services'].sum()
                st.metric("Total Training Gaps", int(total_training_needed))
            
            # District-wise analysis
            if 'district_name' in recommendations_df.columns:
                district_stats = recommendations_df.groupby('district_name').agg({
                    'bsk_id': 'count',
                    'total_training_services': 'sum',
                    'priority_score': 'mean'
                }).reset_index()
                district_stats.columns = ['District', 'BSKs', 'Training Gaps', 'Avg Priority']
                district_stats = district_stats.sort_values('Training Gaps', ascending=False)
    ...
    with tab4:
        ...
        if not recommendations_df.empty:
            ...
            if all_services:
                ...
                top_services = services_rec_df.groupby('service_name')['gap'].sum().nlargest(20)
...
# ============================================================
    ...
    with col1:
        if st.button("🎬 Create Bulk Videos", use_container_width=True):
            st.info("Navigate to Video Generator to create videos for multiple services")
