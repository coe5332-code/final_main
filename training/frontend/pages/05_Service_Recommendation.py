...
try:
    ...
    from ai_service.service_recommendation import (
        recommend_bsk_for_service_from_db,
    ...
    try:
        ...
        AI_FUNCTIONS_AVAILABLE = True
        st.warning(
            f"⚠️ Database not available, using basic recommendation system. Error: {e}"
        )
        ...
        def recommend_bsk_for_service_from_db(
            ...
            try:
                ...
                if not all([services, provisions, bsk_centers]):
                    st.error(
                        "❌ Could not fetch data from API. Please ensure backend is running."
                    )
                    return None
        ...
        def initialize_embeddings_from_db(*args, **kwargs):
            ...
            try:
                ...
                if not services:
                    st.info(
                        "📡 Cannot fetch services data from API for embedding initialization."
                    )
                    return False

                services_df = pd.DataFrame(services)
                initialize_service_embeddings(
                    services_df, force_rebuild=kwargs.get("force_rebuild", False)
                )
        ...
        st.error(
            f"❌ Critical error: Cannot import recommendation functions. Error: {e2}"
        )
...
if "embeddings_initialized" not in st.session_state:
    if AI_FUNCTIONS_AVAILABLE and DATABASE_AVAILABLE:
        with st.spinner("🔄 Initializing recommendation system with database..."):
            try:
                ...
                if success:
                    ...
                    st.success(
                        "  recommendation system with embeddings initialized successfully!"
                    )
                else:
                    st.warning(
                        "⚠️ system initialization incomplete. Continuing without precomputed embeddings."
                    )
        ...
        st.info(
            "💡 Using API-based recommendation system. Embeddings will be computed on-demand."
        )
        st.session_state.embeddings_initialized = False
    else:
...
with st.form("new_service_form"):
    service_name = st.text_input(
        "Service Name", placeholder="e.g., Digital Ration Card Application"
    )
    service_type = st.text_input(
        "Service Type",
        placeholder="e.g., Government Service, Certification, Application",
    )
    service_desc = st.text_area(
        "Service Description",
        placeholder="Describe what this service does, who it's for, and any relevant details...",
    )
    ...
    with col2:
        top_n = st.number_input(
            "Number of recommendations", min_value=1, max_value=1000, value=100
        )
...
if submitted:
        ...
        with st.spinner("🤖 Generating recommendations..."):
            try:
                ...
                if (
                    ...
                    and "recommendations" in recommendations
                ):
                ...
                if recommendations is not None and not recommendations.empty:
                    ...
                    for i, (_, row) in enumerate(top_3.iterrows(), 1):
                        score_emoji = (
                            "🟢"
                            if row["score"] >= 0.7
                            else "🟡" if row["score"] >= 0.4 else "🔴"
                        )
                        st.write(
                            f"{i}. {score_emoji} **{row['bsk_name']}** - Score: {row['score']:.5f}"
                        )
                        if "reason" in row:
                            st.write(f"   💡 {row['reason']}")
                else:
                    st.error(
                        "❌ No recommendations found. Please try adjusting your service description."
                    )
            except ImportError as e:
                st.error(
                    "❌ Database connection issue. Please ensure the backend is running."
                )
                st.code(str(e))
                similar_services = None
            except Exception as e:
    ...
    if (
        similar_services is not None
        and hasattr(similar_services, "__len__")
        and len(similar_services) > 0
    ):
...
if st.session_state.recommendations is not None:
    ...
    with filter_col2:
        ...
        if "district_name" in recommendations.columns:
            districts = ["All"] + sorted(
                recommendations["district_name"].dropna().unique().tolist()
            )
    ...
    if score_range != "All":
        if score_range == "High (≥0.7)":
            filtered_recommendations = filtered_recommendations[
                filtered_recommendations["score"] >= 0.7
            ]
        elif score_range == "Medium (0.4-0.7)":
            filtered_recommendations = filtered_recommendations[
                (filtered_recommendations["score"] >= 0.4)
                & (filtered_recommendations["score"] < 0.7)
            ]
    ...
    st.info(
        f"Showing {len(filtered_recommendations)} of {len(recommendations)} BSKs after filtering"
    )
    ...
    if (
        "bsk_lat" in recommendations_for_map.columns
        and "bsk_long" in recommendations_for_map.columns
    ):
        ...
        with col2:
            dot_size = st.slider(
                "Dot size", min_value=500, max_value=5000, value=2000, step=250
            )
        ...
        if use_clustering and len(map_df) > 50:
            ...
            for cluster_id in range(n_clusters):
                ...
                if len(cluster_data) > 0:
                    ...
                    cluster_info["bsk_name"] = (
                        f"{best_bsk['bsk_name']} (+{len(cluster_data)-1} others)"
                    )
            ...
            st.info(
                f"Clustered {len(recommendations_for_map)} BSKs into {len(map_df)} clusters"
            )
        ...
        if not map_df.empty:
            ...
            if use_clustering and "cluster_size" in map_df.columns:
                ...
                map_df["avg_score_formatted"] = map_df["avg_score"].apply(
                    lambda x: f"{x:.5f}"
                )
                tooltip_html = """
                <b>BSK:</b> {bsk_name}<br/>
        ...
        bsk_options = (
            recommendations_for_map["bsk_name"].tolist()
            if "bsk_name" in recommendations_for_map.columns
            else []
        )
        if bsk_options:
            selected_bsk = st.selectbox(
                "Choose a BSK to view detailed information",
                bsk_options,
                key="bsk_selector",
            )
            ...
            if st.session_state.selected_bsk:
                selected_data = recommendations_for_map[
                    recommendations_for_map["bsk_name"] == st.session_state.selected_bsk
                ]
                if not selected_data.empty:
                    ...
                    with col2:
                        ...
                        if "block_municipalty_name" in row:
                            st.write(
                                f"**Block/Municipality:** {row['block_municipalty_name']}"
                            )
                    with col3:
                        if "reason" in row:
                    ...
                    with st.expander("View all details"):
                        ...
                        detail_cols = [
                            ...
                            if col not in exclude_detail_cols
                        ]
                        st.dataframe(
    ...
    if (
        "bsk_name" in recommendations_for_map.columns
        and not recommendations_for_map.empty
    ):
        tab1, tab2, tab3 = st.tabs(
            ["Score Distribution", "Top BSKs", "Geographic Analysis"]
        )
        ...
        with tab1:
            ...
            bin_labels = [
                ...
                for interval in score_counts.index
            ]
            score_df = pd.DataFrame(
                {"Score Range": bin_labels, "Count": score_counts.values}
            )
            ...
            with col1:
                st.metric(
                    "Mean Score", f"{recommendations_for_map['score'].mean():.5f}"
                )
            with col2:
                st.metric(
                    "Median Score", f"{recommendations_for_map['score'].median():.5f}"
                )
            with col3:
        ...
        with tab3:
            ...
            if "district_name" in recommendations_for_map.columns:
                district_scores = (
                    recommendations_for_map.groupby("district_name")
                    .agg(
                        {
                            "score": ["mean", "count"],
                            "usage_count": (
                                "mean"
                                if "usage_count" in recommendations_for_map.columns
                                else "size"
                            ),
                        }
                    )
