...
#     export_training_recommendations_json,
...
# @app.get("/service_training_recomendation/")
# def service_training_recomendation(limit:int=20,summary_only: bool = False,db: Session = Depends(get_db)):
#     # Fetch all BSKs, provisions, DEOs, and services
#     bsks = db.query(models.BSKMaster).all()
#     provisions = db.query(models.Provision).all()
#     deos = db.query(models.DEOMaster).all()
...
def get_bsk(bsk_id: int, db: Session = Depends(get_db)):
    ...
    bsk = db.query(models.BSKMaster).filter(
        models.BSKMaster.bsk_id == bsk_id
    ).first()
...
@app.get("/services/{service_id}", response_model=ServiceMaster, tags=["Service Master"])
def get_service(service_id: int, db: Session = Depends(get_db)):
    """
    ...
    service = db.query(models.ServiceMaster).filter(
        models.ServiceMaster.service_id == service_id
    ).first()
    ...
    if service is None:
        ...
        raise HTTPException(
            status_code=404, 
            detail=f"Service not found with ID: {service_id}"
        )
...
def get_deo(agent_id: int, db: Session = Depends(get_db)):
    ...
    deo = db.query(models.DEOMaster).filter(
        models.DEOMaster.agent_id == agent_id
    ).first()
    ...
    if deo is None:
        ...
        raise HTTPException(
            status_code=404, 
            detail=f"DEO not found with agent ID: {agent_id}"
        )
...
def get_provisions(
    ...
    """
    logger.info(f"GET /provisions/ - Fetching provisions with skip={skip}, limit={limit}")
...
def get_provision(customer_id: str, db: Session = Depends(get_db)):
    ...
    """
    logger.info(f"GET /provisions/{customer_id} - Fetching provision for customer: {customer_id}")
    
    provision = db.query(models.Provision).filter(
        models.Provision.customer_id == customer_id
    ).first()
...
def get_underperforming_bsks(
    ...
    result_df = find_underperforming_bsks(
        bsks_df, 
        provisions_df, 
        deos_df, 
        services_df
...
def service_training_recommendation(
    limit: int = Query(20, ge=1, le=500, description="Maximum recommendations to return"),
    summary_only: bool = Query(False, description="Return summary instead of detailed recommendations"),
    db: Session = Depends(get_db)
):
    """
